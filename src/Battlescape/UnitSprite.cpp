 ...
 */
UnitSprite::UnitSprite(int width, int height, int x, int y, bool helmet, int bpp) : Surface(width, height, x, y, bpp), _unit(0), _itemA(0), _itemB(0), _unitSurface(0), _itemSurfaceA(0), _itemSurfaceB(0), _part(0), _animationFrame(0), _drawingRoutine(0), _helmet(helmet)
{
}
...
void UnitSprite::drawRoutine0()
{
	...
	if (_unit->getGeoscapeSoldier() && Options::battleHairBleach && torso->getSurface()->format->BitsPerPixel == 8 && legs->getSurface()->format->BitsPerPixel == 8 && leftArm->getSurface()->format->BitsPerPixel == 8 && rightArm->getSurface()->format->BitsPerPixel == 8 && _drawingRoutine == 0)
	{
		SoldierLook look = _unit->getGeoscapeSoldier()->getLook();
