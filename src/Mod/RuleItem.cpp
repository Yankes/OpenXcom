...
void RuleItem::load(const YAML::Node &node, Mod *mod, int listOrder, const ModScript& parsers)
{
	...
	//requires
	mod->loadUnorederedNames(_type, _requiresName, node["requires"]);
	mod->loadUnorederedNames(_type, _requiresBuyName, node["requiresBuy"]);
	...
	mod->loadUnorederedNames(_type, _categories, node["categories"]);
	...
	auto loadAmmoConf = [&](int offset, const YAML::Node &n)
	{
		if (n)
		{
			mod->loadUnorederedNames(_type, _compatibleAmmo[offset], n["compatibleAmmo"]);
	...
	mod->loadUnorederedNames(_type, _supportedInventorySectionsNames, node["supportedInventorySections"]);
